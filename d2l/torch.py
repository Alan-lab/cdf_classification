...
def train_ch6(net, train_iter, test_iter, num_epochs, lr, device):
    ...
    for epoch in range(num_epochs):
        ...
        if(test_acc >= 0.75):
            torch.save(net,'/home/lfq/workspace/classification/model.pkl')
            break     
...
class DotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, valid_lens=None):
        ...
        scores = torch.bmm(queries.transpose(1,2), keys) / math.sqrt(d)
        self.attention_weights = masked_softmax(scores, valid_lens)
        #print(self.attention_weights)
        #print(f'Multi-attention weights {self.attention_weights:}')
        torch.save(self.attention_weights,'/home/lfq/workspace/attention_value.csv')
        return torch.bmm(values, self.dropout(self.attention_weights))
